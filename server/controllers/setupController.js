...
exports.getgameScreen = function(req, res) {
	if (req.params.id == 4){
        ...
        var gameScreen = "<p>Congradulations! You are ready to start on your Journey!</p>"
            + "<p>Here are settings you selected for the game</p>"
            + "<div id=\"returnData\">"
            + "Wagon Leader: <span id=\"rPlayer1Name\">" + leader + "</span><br />"
            + "Members: <span id=\"rPlayer2Name\">" + members.toString() + "</span><br />"
            + "Your profession: <span id=\"rProfession\">" + profession + "</span><br />"
            + "Current bank account: <span id=\"rMoney\">" + money + "</span><br />"
            + "Month leaving: <span id=\"rMonth\">" + startMonth + "</span><br />"
            + "<h2 id=\"pressSpace\">Press the space to continue.</h2>"
            + "</div>";
	}
	else{
...
exports.saveProfession = function(req, res) {
	...
	profession = req.body.playerProfession;
...
exports.saveMemberName = function(req, res) {
	...
	members.push(req.body.memberName);
	gameController.addMember(req.body.memberName);
	res.setHeader('Content-Type', 'text/plain');
	var gameData = gameController.getGame();
